...
const IDMAPS = {
  ...
  MINUS_FROM_CART_BTN: "btn-item-minus",
  PLUS_TO_CART_BTN: "btn-item-plus",
  ITEM_QTY_SPAN: "span-item-qty",
...
const removeFromCart = (item) => {
  const removeFromCartButton = within(item).getByTestId(
    IDMAPS.REMOVE_FROM_CART_BTN
  );
...
const getCartItem = (index, getByTestId) => {
  ...
  const cartItemQuantity =
    within(cartListItem).getByTestId("cart-item-quantity");
...
const getCartDetails = (getByTestId) => {
  return {
    subTotal: parseInt(
      getByTestId(IDMAPS.CART_SUBTOTAL).innerHTML.replace("$", "")
    ),
    discount: parseInt(
      getByTestId(IDMAPS.CART_DISCOUNT).innerHTML.replace("$", "")
    ),
...
test("Should Add item to cart", async () => {
  ...
  minusButton = utils.queryByTestId(IDMAPS.MINUS_FROM_CART_BTN);
  plusButton = utils.queryByTestId(IDMAPS.PLUS_TO_CART_BTN);
  itemQty = utils.queryByTestId(IDMAPS.ITEM_QTY_SPAN);

  expect(removeFromCartBtn).toBeFalsy();
  expect(addToCartButton).toBeTruthy();
  expect(minusButton).toBeFalsy();
  expect(plusButton).toBeFalsy();
  expect(itemQty).toBeFalsy();
  ...
  expect(itemQty.innerHTML).toEqual("1");
  ...
  expect(utils.getByTestId(IDMAPS.CART_ITEM_NAME).innerHTML).toEqual(
    "Hand Bag - $30"
  );
});
...
test("Should recalculate prices when item is removed from cart", () => {
  ...
  expect(cartDetails.discount).toEqual(calculatedDiscount);
  expect(cartDetails.total).toEqual(
    PRODUCTS[3].price + PRODUCTS[2].price - calculatedDiscount
  );
